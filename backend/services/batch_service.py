...
def create_batch(employee_id: str, items: list[dict]) -> tuple[str, str, str, str]:
    ...
    enriched = []
    for item in items:
        price = get_price_for_part(item["part_no"], item["plant"])
        if price is None:
            log_action("BATCH_CREATED", status="FAILURE", employee_id=employee_id,
                       entity="BATCH", detail=f"Part {item['part_no']} not in plant {item['plant']}")
            raise PartNotFoundError(
                f"Part '{item['part_no']}' not found for plant '{item['plant']}'."
            )
...
def submit_batch_to_sap(token: str, orders: list[dict]) -> None:
    """Submit each order in the batch to SAP. Runs as a background task — may take up to 5 min per order."""
    for order in orders:
        try:
            sap_result = call_sap_and_normalize(order["part_no"], order["plant"], order["quantity"])
            store_sap_result(token, sap_result, order_id=order["id"], batch_id=order["batch_id"])
            log_action("SAP_ORDER_SUBMITTED", entity="BATCH", entity_id=token,
                       detail=f"order={order['id']} sap={sap_result.get('order_saved')}")
            logger.info(f"SAP submitted | batch_token={token} order={order['id']}")
        except Exception as e:
            logger.error(f"SAP failed | batch_token={token} order={order['id']}: {e}")
            store_sap_result(token, {}, order_id=order["id"], batch_id=order["batch_id"],
                             api_status="FAILURE", error_detail=str(e))
            log_action("SAP_ORDER_FAILED", status="FAILURE", entity="BATCH",
